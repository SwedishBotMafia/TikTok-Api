...
class TikTokApi:
    ...
    def get_cookies(self, **kwargs):
        did = kwargs.get("custom_did", ''.join(secrets.SystemRandom().choice(string.digits) for num in range(19)))
        if kwargs.get("custom_verifyFp") == None:
            if self.custom_verifyFp != None:
                verifyFp = self.custom_verifyFp
            else:
        ...
        if kwargs.get("force_verify_fp_on_cookie_header", False):
            return {
                ...
                "tt_webid_v2": did,
                "tt_csrf_token": "".join(
                    secrets.SystemRandom().choice(string.ascii_uppercase + string.ascii_lowercase)
                    for i in range(16)
                ),
                "s_v_web_id": verifyFp
            }
    ...
    def getSuggestedUsersbyIDCrawler(
        ...
        while len(users) < count:
            userId = secrets.SystemRandom().choice(unusedIDS)
    ...
    def getSuggestedHashtagsbyIDCrawler(
        ...
        while len(hashtags) < count and len(ids) != 0:
            userId = secrets.SystemRandom().choice(ids)
